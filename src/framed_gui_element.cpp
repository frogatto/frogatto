...
void framed_gui_element::blit(int x, int y, int w, int h, int scale) const
{
	blit_subsection(interior_fill_,x,y,w,h,scale);
	
	blit_subsection(top_border_,x,y,w,top_border_.h(),scale);
	blit_subsection(bottom_border_,x,y + h*scale - bottom_border_.h(),w,bottom_border_.h(),scale);
	blit_subsection(left_border_,x,y,left_border_.w(),h,scale);
	blit_subsection(right_border_,x + w*scale - right_border_.w(), y,right_border_.w(),h,scale);
	
	blit_subsection(top_left_corner_,x,y,top_left_corner_.w(),top_left_corner_.h(),scale);
	blit_subsection(top_right_corner_,x + w*scale - top_right_corner_.w(),y, top_right_corner_.w(), top_right_corner_.h(),scale);
	blit_subsection(bottom_left_corner_,x,y + h*scale - bottom_left_corner_.h(),bottom_left_corner_.w(), bottom_left_corner_.h(),scale);
	blit_subsection(bottom_right_corner_,x + w*scale - bottom_right_corner_.w(),y + h*scale - bottom_right_corner_.h(),bottom_right_corner_.w(), bottom_right_corner_.h(),scale);
...
void framed_gui_element::blit_subsection(rect subsection, int x, int y, int w, int h, int scale) const
{
	graphics::blit_texture(texture_, x, y, w*scale, h*scale, 0.0,
