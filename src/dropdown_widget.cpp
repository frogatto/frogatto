	...
	: widget(v,e), current_selection_(0), dropdown_height_(100)
{
	...
	if(v.has_key("on_change")) {
		...
		on_change_ = boost::bind(&dropdown_widget::change_delegate, this, _1, _2);
	}
...
void dropdown_widget::change_delegate(int selection, const std::string& s)
{
...
void dropdown_widget::text_enter()
{
	...
	if(on_change_) {
		on_change_(current_selection_, editor_->text());
	}
}
...
bool dropdown_widget::handle_event(const SDL_Event& event, bool claimed)
{
	...
	if(dropdown_menu_ && dropdown_menu_->visible()) {
		SDL_Event ev = event;
		normalize_event(&ev);
		dropdown_menu_->handle_event(ev, claimed);
		if(claimed) {
			return claimed;
		}
	}
...
void dropdown_widget::execute_selection(int selection)
{
	...
	if(on_change_) {
		if(type_ == DROPDOWN_LIST) {
			on_change_(current_selection_, list_[current_selection_]);
		} else if(type_ == DROPDOWN_COMBOBOX) {
			on_change_(current_selection_, editor_->text());
		}
	}
...
variant dropdown_widget::get_value(const std::string& key) const
{
	if(key == "selection") {
		...
		if(current_selection_ < 0 || current_selection_ > list_.size()) {
			return variant();
		}
