objects = background.o button.o character.o character_editor_dialog.o character_type.o color_utils.o custom_object.o custom_object_functions.o custom_object_type.o dialog.o draw_number.o draw_scene.o draw_tile.o editor.o entity.o filesystem.o fluid.o font.o formula.o formula_function.o formula_tokenizer.o frame.o geometry.o graphical_font.o grid_widget.o gui_section.o image_widget.o input.o item.o item_type.o inventory.o joystick.o key.o label.o level.o level_logic.o level_object.o load_level.o main.o message_dialog.o powerup.o preferences.o preprocessor.o preview_tileset_widget.o prop.o prop_editor_dialog.o raster.o sound.o speech_dialog.o string_utils.o surface_cache.o surface_formula.o surface_scaling.o surface.o texture.o tile_map.o tileset_editor_dialog.o tooltip.o translate.o variant.o water.o widget.o wml_formula_adapter.o wml_modify.o wml_node.o wml_parser.o wml_schema.o wml_utils.o wml_writer.o unit_test.o formula_test.o wml_parser_test.o
...
game: $(objects)
	g++ -g $(OPT) -L/sw/lib -L/usr/X11R6/lib -lX11 -D_GNU_SOURCE=1 -D_REENTRANT -Wnon-virtual-dtor -Wreturn-type -L/usr/lib `sdl-config --libs` -lSDLmain -lSDL -lGL -lGLU -lSDL_image -lSDL_ttf -lSDL_mixer -lboost_regex -lboost_thread-mt -fthreadsafe-statics *.o -o game
