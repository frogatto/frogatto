...
#<Jetrel_bot> marcavis:  reverse means it plays frames 1,2,3,4,3,2,1
#<Jetrel_bot> plays_backwards means it plays frames 4,3,2,1
#<Jetrel_bot> both, together, mean it plays frames 4,3,2,1,2,3,4
...
def mainRead(cfgFile, anim):
	...
	reverse = any(["reverse" in x and "true" in x for x in anim.split('\n')])
	play_backwards = any(["play_backwards" in x and "true" in x for x in anim.split('\n')])
	...
	frameFilenames = " ".join(["make-gif-temp" + ("%03d" % x) + ".png" for x in frameIDs])
	...
	makeAnim = "convert -scale " + str(scale) + "00% -extent 560x440 -gravity center" + \
				...
				" -layers optimize " + frameFilenames + " make-gif-output.gif"
	print makeAnim
	bash(makeAnim)
